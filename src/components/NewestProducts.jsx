...
export default function Newest() {
  ...
  useEffect(() => {
    fetch("http://zinopetstore.ir/api/products/newestProductsAPI.php?page=1&count=10")
      .then((res) => res.json())
      .then((res) => setNewestProducts(res.productsInfo));
  }, []);
  ...
  const items = newestProducts.map((item) => (
  ...
  return (
    <div className="newest-products">
      <h2 className="newest-products-title"> جدید ترین محصولات</h2>
      <AliceCarousel
        mouseTracking
        disableDotsControls
        items={items}
